...
tf.app.flags.DEFINE_string('train_data_path', "kcws_data/train.txt",
                           'Training data dir')
tf.app.flags.DEFINE_string('test_data_path', "kcws_data/test.txt",
                           'Test data dir')
tf.app.flags.DEFINE_string('debug_data_path', "data/debug.txt", "")
tf.app.flags.DEFINE_string('log_dir', "logs", 'The log  dir')
tf.app.flags.DEFINE_string("word2vec_path", "kcws_data/vec.txt",
...
class Model:
    def __init__(self, embeddingSize, distinctTagNum, c2vPath, numHidden):
        ...
        c2v_model = w2v.load(c2vPath)
        self.lexicon = {w:i for i, w in enumerate(c2v_model.vocab)}
...
break_marks = set(["。", ",", "，", " ", "\t", "?", "？", "!",
       "！", ";", "；"])
pair_marks_reverse = {v:k for k, v in pair_marks.items()}
...
def split_text(sentence):
        ...
        for idx in range(len(sentence)):
            char = sentence[idx]
            if char in pair_marks:
                state = 1
                if start < idx: # for case [][]
                    results.append(sentence[start:idx])
                start = idx
            elif char in pair_marks_reverse and state == 1:
                if pair_marks_reverse[char] == sentence[start]:
                    state = 0
                    results.append(sentence[start:idx+1])
                    start = idx + 1
            elif char in break_marks:
                results.append(sentence[start:idx+1])
                start = idx + 1
...
def do_debug(logits, test_real_length, sess, lexicon,
            transition_params, debug_data_path, inp):
        ...
        with open(debug_data_path, "r") as f:
            for line in f:
                sentence = line.strip()
                split_sentences = split_text(sentence)

                x_inputs_val = np.zeros([len(split_sentences),
                    FLAGS.max_sentence_len], dtype="int32")
        ...
        with sv.managed_session(master='') as sess:
            ...
            for step in range(training_steps):
                ...
                try:
                    ...
                    if (step + 1) % 20 == 0:
                        do_debug(test_unary_score, test_sequence_length,
                                sess, model.lexicon, trainsMatrix,
                                FLAGS.debug_data_path, model.inp)
